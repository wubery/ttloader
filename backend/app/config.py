...
class Settings(BaseSettings):
    ...
    # Через сколько дней выполненная задача уходит из очереди в счётчик статистики
    # (вместе со своим рендером и скриншотами). Упавшие не трогаются никогда.
    jobs_keep_days: int = 3
    ...
    # 16–18 — визуально «без потерь» для соцсетей: TikTok перекодирует ролик у себя
    # ещё раз, и всё, что потеряно на первом проходе, обратно не появится.
    # preset влияет на скорость: veryfast ≈ втрое быстрее medium, но заметно мягче
    # на движении. Быстрее и хуже — VIDEO_PRESET=veryfast в .env.
    video_crf_min: int = 16
    video_crf_max: int = 18
