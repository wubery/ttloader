...
export const api = {
  ...
  createAccount: (b: { name: string; platform: Platform; proxy_url?: string | null }) =>
    fetch("/api/accounts", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(b),
    }).then((r) => j<Account>(r)),
  updateAccount: (id: number, b: Partial<{ name: string; proxy_url: string | null; active: boolean }>) =>
  ...
  updateBanner: (id: number, b: Partial<Pick<Banner, "name" | "x" | "y" | "scale" | "opacity">>) =>
